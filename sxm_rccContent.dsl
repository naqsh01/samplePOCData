    ...
    if (successCount > 0) {
        ...
        successCount.times {
            sendReportingData reportObjectTypeName: 'build', payload: """{
                ...
                "baseDrilldownUrl": "http://10.200.1.171:8081/job/EC%20Reporting%20Plugins"
            }"""
        }
    }
...
def loadTests(def category, def endTime, def durationInMillis, def successCount, def failureCount) {
    ...
    sendReportingData reportObjectTypeName: 'quality', payload: """{
        ...
        "baseDrilldownUrl": "http://10.200.1.171:8081/job/EC%20Reporting%20Plugins/lastBuild/"
    }"""
}
    ...
    if (openCount > 0) {
        ...
        openCount.times {
            ...
            sendReportingData reportObjectTypeName: 'feature', payload: """{
                "source": "JIRA",
                "sourceUrl": "http://10.200.1.171:8080/browse/${it}",
                "pluginName": "EC-JIRA",
                "pluginConfiguration": "config",
                "releaseName": "$args.releaseName",
                "releaseProjectName": "$args.projectName",
                ...
                "status": "$status",
                "timestamp": "$timeStr",
                "baseDrilldownUrl": "http://10.200.1.171:8080/browse/REP-19?jql=project%20%3D%20REP%20AND%20issuetype%20%3D%20Story",
                "releaseUri": "AND%20fixVersion%20IN('${args.releaseName}')"
            }"""
        }
    }
